  ...
  return (
    <div className="campaign-manager-page">
      <PageShell
        ...
        subtitle="View, edit, pause, schedule, and manage every brand campaign"
        actions={
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <DataPanel title={camp.brandName || 'Campaign'} live>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 12 }}>
                {!details.isActive && (
                  <button className="btn primary" onClick={() => activate(camp.id)}>Set Active</button>
                )}
                {details.isPaused || camp.status === 'Paused' ? (
                  <button className="btn primary" onClick={() => resume(camp.id)}>Resume</button>
                ) : (
                  <button className="btn" onClick={() => pause(camp.id)}>Pause</button>
                )}
                <button className="btn" onClick={() => setEditing((e) => !e)}>{editing ? 'Cancel Edit' : 'Edit'}</button>
                ...
                <button className="btn" style={{ color: '#ef4444', borderColor: '#ef4444' }} onClick={() => remove(camp.id)}>Delete</button>
              </div>
            </DataPanel>
            ...
            <DataPanel title={`Scheduled Posts (${details.scheduledPosts?.length ?? 0})`} live>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, minWidth: 180 }}>
                          ...
                          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                            <button className="btn" onClick={() => reschedulePost(p.id)}>Reschedule</button>
                            <button className="btn" onClick={() => publishNow(p.id)}>Publish now</button>
              ...
              <div className="card" style={{ marginTop: 12 }}>
                ...
                <button className="btn primary" style={{ marginTop: 8 }} onClick={scheduleNewPost}>Schedule Post</button>
              </div>
            </DataPanel>
          </div>
        )}
