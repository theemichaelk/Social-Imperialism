...
export const INTEGRATION_GROUPS: IntegrationGroup[] = [
  {
    ...
    fields: [
      ...
      { key: 'falKey', label: 'FAL AI Key (image + video gen)', placeholder: 'FAL key', type: 'password', metric: 'FAL', hint: 'Unlocks FLUX images, Kling/Veo/MiniMax video in OpenMontage' },
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'siSerpBaseUrl', label: 'Social Imperialism SERP — Base URL', type: 'url', metric: 'Social Imperialism SERP', placeholder: 'http://127.0.0.1:7000', hint: 'Local OpenSERP sidecar (Google, Bing, Yahoo, Yandex, Baidu, DuckDuckGo, Ecosia). Desktop auto-starts when SI_SERP_AUTOSTART=true. Preferred over SerpAPI when set.' },
      { key: 'siSerpApiKey', label: 'Social Imperialism SERP — API Key', type: 'password', metric: 'Social Imperialism SERP', hint: 'Optional bearer token for your SERP API host.' },
      { key: 'serpApiKey', label: 'SerpAPI Key (legacy fallback)', type: 'password', metric: 'SerpAPI' },
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'acumbamailHost', label: 'Acumbamail SMTP Server', type: 'text', metric: 'Acumbamail', placeholder: 'smtp.acumbamail.com', hint: 'Port 587/25252/25 (StartTLS) or 2525/465 (TLS). Contact Acumbamail support to activate SMTP relay.' },
...
export const LIVE_INTEGRATION_TESTS: LiveIntegrationTest[] = [
  ...
  { id: 'serp', label: 'SERP Research (Social Imperialism / SerpAPI)', channel: 'serp-search', args: [{ query: 'social media automation', limit: 5 }], category: 'seo', metric: 'Social Imperialism SERP' },
...
export const METRIC_PROBE_MAP: Record<string, { channel: string; args?: unknown[] }> = {
  ...
  'SerpAPI': { channel: 'serp-search', args: [{ query: 'social media automation', limit: 5 }] },
  'Social Imperialism SERP': { channel: 'serp-search', args: [{ query: 'social media automation', limit: 5, extract: 1 }] },
