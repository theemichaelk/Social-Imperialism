...
const newAvail = `ipcMain.handle('get-available-accounts', async (event, credentials) => {
    // If it's Facebook, we'll try to fetch real pages using the API key if available
    if (credentials.platform === 'Facebook') {
        ...
        if (metaKey) {
            try {
                ...
                // Use Meta Graph API to fetch user's profile and their managed pages
                const profileRes = await fetch(\`https://graph.facebook.com/v19.0/me?fields=id,name&access_token=\${metaKey}\`);
                const pagesRes = await fetch(\`https://graph.facebook.com/v19.0/me/accounts?fields=name,id,category&access_token=\${metaKey}\`);
                const groupsRes = await fetch(\`https://graph.facebook.com/v19.0/me/groups?fields=name,id,privacy&access_token=\${metaKey}\`);
...
if (indexJs.match(regexAvail)) {
    ...
    console.log("Updated FB/YT account pulling logic.");
} else {
    console.log("Could not find the handler block.");
}