...
<head>
  ...
  <style>
    ...
    .sidebar-header h2 { margin: 0; font-size: 1.1rem; color: #38bdf8; display: flex; align-items: center; gap: 10px; cursor: pointer; text-transform: uppercase; letter-spacing: 1px; }
...
<body>
  ...
  <div class="sidebar">
    <div class="sidebar-header">
      <h2 onclick="window.location.href='index.html'"><i class="fas fa-arrow-left" style="font-size:0.9rem;"></i> Builder</h2>
    </div>
  ...
  <div class="main-area">
    <div class="topbar">
      <div style="display:flex; align-items:center; gap:20px;">
        <h3 style="margin:0; font-size:1.1rem; font-weight:600; letter-spacing:0.5px;">Campaign Pipeline <span style="font-size:0.8rem; color:#64748b; font-weight:400;">/ Draft</span></h3>
        <div class="template-selector">
          <button class="template-btn" onclick="loadTemplate('engagement')"><i class="fas fa-magic"></i> Auto-Engagement Template</button>
          <button class="template-btn" onclick="loadTemplate('rss')"><i class="fas fa-rss"></i> RSS to Social Template</button>
        </div>
      </div>
      <div style="display:flex; gap:12px;">
        <button class="btn btn-secondary" onclick="clearCanvas()"><i class="fas fa-trash"></i> Clear</button>
        <button class="btn btn-primary" onclick="savePipeline()"><i class="fas fa-rocket"></i> Deploy Flow</button>
      </div>
    </div>
...
<script>
  ...
  function createNode(type, title, icon, x, y, id = null, config = {}) {
    ...
    } else if (type !== 'action' || title.includes('Publish')) {
  ...
  function selectNode(id, e) {
    ...
    if(node.type === 'trigger') {
      ...
      if(node.title.includes('RSS')) {
          ...
          html += '<div class="form-group"><label>Webhook URL (Generated)</label><input type="text" readonly value="https://api.socialimperialism.local/hook/hw82kd9"></div>';
  ...
  window.loadTemplate = function(type) {
      if(!confirm("Loading a template will clear your current canvas. Continue?")) return;
      Object.values(nodes).forEach(n => n.el.remove());
      ...
      if(type === 'engagement') {
          ...
          connections.push({from: n2, to: n3, port: 'out-true'}); // Positive path
          connections.push({from: n2, to: n5, port: 'out-false'}); // Negative path
          ...
          nodes[n2].title = "AI Summarize"; // modify title
  ...
  // Initial Load
  setTimeout(() => loadTemplate('engagement'), 300);
...
console.log("Visual Automations Builder Tweaked & Overwritten Successfully");