...
const PAGES = [
  ...
  },
  {
    ...
    name: 'Video Studio',
    features: [
      { name: 'Studio config', channel: 'get-imperial-video-studio-config', validate: (d) => d?.pipelineCount === 12 && d?.toolCount === 52 },
      { name: 'Tool registry', channel: 'get-imperial-video-tool-registry', validate: (d) => d?.success === true && d?.toolCount === 52 },
      { name: 'Skills index', channel: 'get-imperial-video-skills-index', validate: (d) => d?.success === true && d?.count >= 500 },
