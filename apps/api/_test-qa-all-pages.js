...
const PAGES = [
  ...
  },
  {
    ...
    name: 'Video Studio',
    features: [
      { name: 'Studio config', channel: 'get-imperial-video-studio-config', validate: (d) => d?.pipelineCount === 12 && d?.toolCount === 52 && d?.skillCount === 620 },
      { name: 'Tool registry', channel: 'get-imperial-video-tool-registry', validate: (d) => d?.success === true && d?.toolCount === 52 && d?.skillCount === 620 },
      { name: 'Skills index', channel: 'get-imperial-video-skills-index', validate: (d) => d?.success === true && d?.count === 620 },
      { name: 'Reference analyze', channel: 'analyze-reference-video', args: [{ url: 'https://example.com/short', topic: 'QA video' }], validate: (d) => d?.success === true },
      { name: 'Quick pipeline', channel: 'run-imperial-video-pipeline', args: [{ pipelineId: 'social-explainer', quick: true, topic: 'QA' }], validate: (d) => d?.success === true && d?.stageCount === 7 },
      { name: 'Pipeline result', channel: 'get-imperial-video-pipeline-result', validate: (d) => d?.success !== false },
      { name: 'Clear pipeline', channel: 'clear-imperial-video-pipeline-result', validate: (d) => d?.success === true && d?.status === 'idle' },
      { name: 'Grok status', channel: 'grok-get-status', validate: (d) => typeof d === 'object' },
    ],
  },
  {
